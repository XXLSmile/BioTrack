...
class FriendRepository @Inject constructor(
    ...
    suspend fun fetchFriends(): Result<FriendListResponse> = runCatching {
        ...
        if (response.isSuccessful) {
            ...
            throw FriendRepositoryException(
                response.errorBody()?.string().orEmpty().ifBlank { "Failed to load friends" }
            )
        }
    }
    ...
    suspend fun fetchFriendRequests(type: String? = null): Result<FriendRequestsResponse> = runCatching {
        ...
        if (response.isSuccessful) {
            ...
            throw FriendRepositoryException(
                response.errorBody()?.string().orEmpty().ifBlank { "Failed to load friend requests" }
            )
        }
    }
    ...
    suspend fun sendFriendRequest(targetUserId: String): Result<Unit> = runCatching {
        ...
        if (!response.isSuccessful) {
            throw FriendRepositoryException(
                response.errorBody()?.string().orEmpty().ifBlank { "Failed to send friend request" }
            )
        }
    }
    ...
    suspend fun respondToRequest(requestId: String, action: String): Result<Unit> = runCatching {
        ...
        if (!response.isSuccessful) {
            throw FriendRepositoryException(
                response.errorBody()?.string().orEmpty().ifBlank { "Failed to update friend request" }
            )
        }
    }
    ...
    suspend fun cancelFriendRequest(requestId: String): Result<Unit> = runCatching {
        ...
        if (!response.isSuccessful) {
            throw FriendRepositoryException(
                response.errorBody()?.string().orEmpty().ifBlank { "Failed to cancel friend request" }
            )
        }
    }
    ...
    suspend fun removeFriend(friendshipId: String): Result<Unit> = runCatching {
        ...
        if (!response.isSuccessful) {
            throw FriendRepositoryException(
                response.errorBody()?.string().orEmpty().ifBlank { "Failed to remove friend" }
            )
        }
    }
    ...
    suspend fun searchUsers(query: String): Result<SearchUsersResponse> = runCatching {
        ...
        if (response.isSuccessful) {
            ...
            throw FriendRepositoryException(
                response.errorBody()?.string().orEmpty().ifBlank { "Failed to search users" }
            )
        }
    }
    ...
    suspend fun fetchFriendRecommendations(limit: Int? = null): Result<List<FriendRecommendation>> = runCatching {
        ...
        if (response.isSuccessful) {
            ...
            throw FriendRepositoryException(
                response.errorBody()?.string().orEmpty().ifBlank { "Failed to load friend recommendations" }
            )
        }
    }
