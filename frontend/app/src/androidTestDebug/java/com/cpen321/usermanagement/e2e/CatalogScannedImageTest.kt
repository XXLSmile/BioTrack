...
class CatalogScannedImageTest {
    ...
    fun catalogScannedImageTest() {
        ...
        // Handle location permission popup (if appears)
        val allowWhileUsing = device.findObject(UiSelector().textContains("While using the app"))
        ...
        // Wait briefly for dialog to close and app to resume
        Thread.sleep(3000)

        // Click Identify again (actual scan trigger)
        composeRule.onNodeWithText("Recognize Animal").performClick()

        // Wait for “Save to Catalog” dialog
        composeRule.waitUntil(timeoutMillis = 20_000) {
            composeRule.onAllNodesWithText("Save to Catalog").fetchSemanticsNodes().isNotEmpty()
        }

        // Select first catalog in dialog
        composeRule.onNodeWithText("Save to Catalog").assertExists()
