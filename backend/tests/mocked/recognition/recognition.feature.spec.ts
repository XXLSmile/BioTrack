...
import { createUserAndToken, createUserAndTokenWithPayload } from '../../unmocked/auth/helpers';
...
describe('API (mocked): /api/recognition endpoints', () => {
  beforeEach(async () => {
  ...
  test('POST /api/recognition surfaces timeout responses', async () => {
    mockedRecognitionService.recognizeFromUrl.mockRejectedValueOnce(
      new Error('Request timed out while calling Zyla'),
    );
  ...
  test('POST /api/recognition/save handles image relocation when original path not found', async () => {
    ...
    const relocatedPath = path.join(uploadsDir, 'original-test.jpg');
    ...
    const safeFs = require('../../../src/utils/safeFs') as {
      existsSync: (filePath: string) => boolean;
    };
    const originalExists = safeFs.existsSync.bind(safeFs);
    const uploadsRoot = path.resolve(path.join(__dirname, '../../..', 'uploads'));
    const originalFullPath = path.resolve(uploadsRoot, 'tmp', 'original-test.jpg');
    const relocatedFullPath = path.resolve(uploadsRoot, 'images', 'original-test.jpg');
    jest.spyOn(safeFs, 'existsSync').mockImplementation((filePath: string) => {
      const resolvedPath = path.resolve(filePath);
      if (resolvedPath === originalFullPath) return false;
      if (resolvedPath === relocatedFullPath) return true;
    ...
    expect(response.body?.message).toBe('Species recognized and saved successfully');
